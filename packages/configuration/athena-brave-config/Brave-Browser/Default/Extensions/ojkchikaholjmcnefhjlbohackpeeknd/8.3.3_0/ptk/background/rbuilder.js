...
export class ptk_request_manager {
    ...
    async msg_scan_request(message) {
        worker.ptk_app.rattacker.scanRequest(message.schema, false).then(scanResult => {
...
export class ptk_request {
    ...
    parseRawRequest(requestObject) {
        ...
        requestModel.followRedirect = requestObject.request_redirect == 'on' ? true : false
        requestModel.overwriteHeaders = requestObject.request_headers_overwrite == 'on' ? true : false
        if (requestModel.host == 'unspecified-host') {
            try {
                let url = new URL(requestModel.target)
                requestModel.host = url.host
    ...
    async sendRequest(schema) {
        ...
        let ruleId = null
        if (schema.request.overwriteHeaders && schema.request.headers.length > 0) {

            ruleId = await ptk_ruleManager.addSessionRule(schema)
            console.log(ruleId)
            this.trackingRequest = new Map()
            let oHeaders = JSON.parse(JSON.stringify(schema.request.headers))
            if (oHeaders.findIndex(x => x.name.toLowerCase() == 'host') < 0) {
                oHeaders.push({ name: 'Host', value: schema.request.host })
            }
            this.trackingRequest.set('originalHeaders', oHeaders)
        }
        ...
        return fetch(schema.request.target, params).then(async function (response) {
            ...
            if ((!response.ok || !trackingRequest) && response.type != 'opaqueredirect') {
                let h = []
