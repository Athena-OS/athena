...
jQuery(function () {
    ...
    async function bindInfo() {
        if (index_controller.url) {
            $('#dashboard_message_text').html('<h2>PTK report:</h2>  ' + index_controller.url)
        } else {
    ...
    function generateRattacker() {
        rattacker_controller.init().then(function (result) {
            if (index_controller.tabId == result?.scanResult?.scannedTabId) {
                ...
                document.getElementById("rattacker_content").innerHTML = report
            }
    ...
    if (params.has('rattacker_report')) {
        ...
        browser.storage.local.get('rattacker_report').then(function (r) {
            let dec = CryptoES.enc.Utf8.stringify(CryptoES.enc.Base64.parse(r.rattacker_report))
            document.getElementById("rattacker_content").innerHTML = dec
            $('.loader.rattacker').hide()
        })
