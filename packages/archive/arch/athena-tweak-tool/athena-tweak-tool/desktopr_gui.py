...
def gui(self, Gtk, GdkPixbuf, vboxstack12, desktopr, fn, base_dir, Pango):
    ...
    checkbox = Gtk.Box(orientation=Gtk.Orientation.VERTICAL, spacing=10)
...
the Athena repository"
    ...
    self.d_combo_desktop = Gtk.ComboBoxText()
    self.d_combo_desktop.set_size_request(180, 0)
    self.d_combo_desktop.connect("changed", self.on_d_combo_changed_desktop)
    for x in desktopr.desktops:
        self.d_combo_desktop.append_text(x)
    ...
    # set_default.connect("clicked", self.on_default_clicked_desktop)
    ...
    self.ch2 = Gtk.CheckButton(label="Select to clear environment configuration files")
    checkbox.pack_start(self.ch2, False, False, 0)
    # =======================================
    #               TEXTVIEW
    # =======================================
