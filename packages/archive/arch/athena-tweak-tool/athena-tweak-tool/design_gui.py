...
def gui(self, Gtk, GdkPixbuf, vboxstack24, design, fn, base_dir, Pango):
    """create a gui"""
    ...
    lbl1.set_text("Design Installer")
    lbl1.set_name("title")
...
- see Pacman tab</b>\n\nSome of the designs can only be installed if we can access \n\
the Athena repository"
    )
    label = Gtk.Label(xalign=0)
    label.set_text("Select a design")
    ...
    self.d_combo_design = Gtk.ComboBoxText()
    self.d_combo_design.set_size_request(180, 0)
    self.d_combo_design.connect("changed", self.on_d_combo_changed_design)
    for x in design.designs:
        self.d_combo_design.append_text(x)
    self.d_combo_design.set_active(0)
    self.d_combo_design.set_wrap_width(1)
    ...
    dropbox.pack_start(self.d_combo_design, False, False, 0)

    # =======================================
    #               STATUS
    # =======================================
    statbox.pack_start(self.design_status, True, False, 0)

    # =======================================
    #               BUTTONS
    # =======================================

    self.button_install_design = Gtk.Button(label="Install")
    self.button_reinstall_design = Gtk.Button(label="Re-Install")
    ...
    self.button_install_design.connect("clicked", self.on_install_clicked_design, "inst")
    self.button_reinstall_design.connect("clicked", self.on_install_clicked_design, "reinst")

    buttonbox.pack_start(self.button_install_design, True, True, 0)
    buttonbox.pack_start(self.button_reinstall_design, True, True, 0)
    ...
    # set_default.connect("clicked", self.on_default_clicked_design)
    ...
    #               TEXTVIEW
    # =======================================
    self.design_prog = Gtk.ProgressBar()
    self.design_stat = Gtk.Label(xalign=0)
    self.design_stat.set_ellipsize(Pango.EllipsizeMode.MIDDLE)
...
Know that these packages conflict with picom-git. It will be removed."
    ...
    self.design_error = Gtk.Label(xalign=0)
    ...
    vboxprog.pack_start(self.design_error, False, False, 0)
    vboxprog.pack_start(self.design_stat, False, False, 0)
    vboxprog.pack_start(self.design_prog, False, False, 0)
